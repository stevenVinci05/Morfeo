...
class LoopEvents(commands.Cog):
    ...
    async def on_message(self, message):
        bannedWorlds = ReadFile(config.JSON_BANNED)
        ...
        if any(parola in contenuto for parola in bannedWorlds):
            try:
                await message.delete()
                await message.channel.send(f"🚫 {message.author.mention}, il tuo messaggio conteneva una parola vietata.", delete_after=5)
            except Exception as e:
